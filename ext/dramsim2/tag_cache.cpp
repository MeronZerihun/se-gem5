...
TagCache::TagCache(void* dram, uint32_t size, uint32_t access_latency){
    this->dram = (DRAMSim::MultiChannelMemorySystem*) dram;
...
void TagCache::write_done_callback(){
	assert(outstanding_requests > 0);
	outstanding_requests --;
}


void TagCache::add_response_schedule(uint64_t earliest_cycle, uint64_t address){
...
void TagCache::process_dram_response(){
    
    if( !dram->isTagRespAvailable() ) return;

    uint64_t address = dram->getTagResponse();
    ...
    if( mshr_entry != mshr.end() ){
        ...
        if(is_write){
            ...
            MEE_DEBUG("cache_schedule\t" << current_cycle+CACHE_RESPONSE_CYCLES);
            add_response_schedule(current_cycle + CACHE_RESPONSE_CYCLES, address);    
        }
...
void TagCache::evict_block(uint16_t way){
...
void TagCache::send_dram_req(){
    ...
    if(dram->channels[chan]->WillAcceptTransaction() && outstanding_requests < 16 ){
        assert( dram->addTaggedTransaction(do_write, true, addr) );
...
void TagCache::add_input(bool is_write, uint64_t address){
    
    MEE_DEBUG("tag_request\t0x" << hex << address);
...
bool TagCache::can_accept_input(){
